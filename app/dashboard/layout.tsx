...
export default function DashboardLayout({
  ...
  const menuItems = [
    { name: "Dashboard", icon: LayoutDashboard, path: "/dashboard" },
    {
      ...
      subItems: [
        ...
        { name: "Sub-Categories", path: "/dashboard/sub-categories" },
      ],
    },
    { name: "Price Management", icon: Tag, path: "/dashboard/price-management" },
    { name: "Product Management", icon: Package, path: "/dashboard/product-management" },
    { name: "Payment Details", icon: CreditCard, path: "/dashboard/payment-details" },
    { name: "Cashier", icon: UserCog, path: "/dashboard/cashier" },
    { name: "Inventory", icon: Warehouse, path: "/dashboard/inventory" },
    { name: "Store Management", icon: Store, path: "/dashboard/store-management" },
    { name: "Print Management", icon: Printer, path: "/dashboard/print-management" },
    { name: "Tax/GST Management", icon: ReceiptText, path: "/dashboard/tax-gst-management" },
    {
      name: "Customer Management",
      icon: Users,
      subItems: [
        { name: "Customer Profiles", path: "/dashboard/customer-profiles" },
        { name: "Purchase History", path: "/dashboard/purchase-history" },
        { name: "Loyalty Programs", path: "/dashboard/loyalty-programs" },
        { name: "Coupons and gift cards", path: "/dashboard/coupons-and-gift-cards" },
        { name: "Personalized Offers", path: "/dashboard/personalized-offers" },
      ],
    },
  ];
  ...
  return (
    <div className="flex min-h-screen overflow-hidden bg-gray-50">
      ...
      >
        ...
        <nav className={`flex-1 space-y-1 py-4 ${isSidebarOpen ? "px-3" : "px-2"}`}>
          {menuItems.map((item) => {
            ...
            if (item.subItems) {
              const isExpanded = item.name === "Category Management" 
                ? isCategoryOpen 
                : isCustomerOpen;
              
              const toggleExpand = item.name === "Category Management" 
              ...
              return (
                <div key={item.name}>
                  <button
                    ...
                    className={`group relative flex w-full items-center justify-between rounded-lg px-3 py-2.5 transition-all duration-200 ${!isSidebarOpen ? "justify-center" : ""} cursor-pointer ${
                      isThisItemActive
                        ? "bg-[#622581]/10 text-[#622581]" 
                        : "text-gray-700 hover:bg-[#622581]/10 hover:text-[#622581]"
                    }`}
                  >
                    <span className={`absolute left-0 top-0 bottom-0 w-1 bg-[#622581] rounded-r-full transition-opacity duration-200 ${
                      isThisItemActive ? "opacity-100" : "opacity-0 group-hover:opacity-100"
                    }`}></span>
                    
                    <div className="flex items-center gap-3">
                      <item.icon className="w-5 h-5 transition-colors duration-200" />
                      {isSidebarOpen && <span className="font-nunito font-medium text-sm">{item.name}</span>}
                    </div>
            ...
            return (
              <Link
                ...
                title={!isSidebarOpen ? item.name : undefined}
                className={`group relative flex items-center gap-3 rounded-lg px-3 py-2.5 transition-all duration-200 ${!isSidebarOpen ? "justify-center" : ""} cursor-pointer ${
                  isThisItemActive
                    ? "bg-[#622581]/10 text-[#622581]"
                    : "text-gray-700 hover:bg-[#622581]/10 hover:text-[#622581]"
                }`}
      ...
      <div className={`flex min-h-screen min-w-0 flex-1 flex-col transition-all duration-200 ${!isSidebarOpen ? "lg:ml-0" : ""}`}>
        {/* Navbar / Header - Exact match to image */}
        <header className="sticky top-0 z-10 flex items-center justify-between border-b border-gray-200 bg-white px-3 py-3 sm:px-6">
        ...
        {/* Page Content */}
        <main className="scrollbar-none min-w-0 flex-1 scroll-smooth overflow-y-auto p-3 sm:p-6">
          {children}
        </main>
      </div>
    </div>
