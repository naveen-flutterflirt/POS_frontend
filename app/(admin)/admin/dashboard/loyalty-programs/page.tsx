  ...
  return (
    <div className="h-full max-h-full space-y-6 overflow-hidden font-nunito">
      <div className="flex items-start justify-between gap-4">
        ...
        <Link href="/admin/dashboard/loyalty-programs/create" className="mt-1 flex min-w-[143px] items-center justify-center rounded-lg bg-[#622581] px-4 py-3 text-sm font-nunito font-semibold text-white transition-colors hover:bg-[#52206d]">Add Loyalty Programs</Link>
      </div>
      ...
      <section className="h-[calc(100vh-190px)] min-h-0 overflow-hidden border border-gray-100 bg-white">
        ...
        <div className="overflow-hidden text-xs font-normal text-gray-800">
          <div className="grid h-11 grid-cols-[10%_15%_10%_15%_11%_13%_13%_13%] items-center bg-[#f3f3f3] text-left"><div className="px-3 whitespace-nowrap">Program Id</div><div className="px-3 whitespace-nowrap">Program Name</div><div className="px-3 whitespace-nowrap">Type</div><div className="px-3 whitespace-nowrap">Redemption Rule</div><div className="px-3 whitespace-nowrap">Status</div><div className="px-3 whitespace-nowrap">Start Date</div><div className="px-3 whitespace-nowrap">End Date</div><div className="px-3 text-center whitespace-nowrap">Actions</div></div>
          {programs.map((program) => <div key={program.id} className="grid h-[50px] grid-cols-[10%_15%_10%_15%_11%_13%_13%_13%] items-center border-b border-[#e2e6eb]"><div className="px-3 whitespace-nowrap">{program.programId}</div><div className="px-3 whitespace-nowrap">{program.name}</div><div className="px-3 whitespace-nowrap">{program.type}</div><div className="px-3 whitespace-nowrap">{program.redemptionRule}</div><div className="px-3"><span className={`inline-flex rounded px-3 py-1 text-[11px] text-white ${program.status === "Active" ? "bg-[#55b566]" : "bg-[#e49a49]"}`}>{program.status}</span></div><div className="px-3 whitespace-nowrap">{program.startDate}</div><div className="px-3 whitespace-nowrap">{program.endDate}</div><div className="px-3"><div className="flex items-center justify-center gap-3"><button type="button" onClick={() => handleEditClick(program)} className="rounded p-1 text-[#1463ff] hover:bg-blue-50" aria-label={`Edit ${program.name}`}><Edit className="h-[18px] w-[18px]" /></button><button type="button" onClick={() => { setSelectedProgram(program); setIsDeleteModalOpen(true); }} className="rounded p-1 text-[#ff0000] hover:bg-red-50" aria-label={`Delete ${program.name}`}><Trash2 className="h-[18px] w-[18px]" /></button></div></div></div>)}
        </div>
      </section>
