  ...
  return (
    <div className="space-y-4 font-nunito">
      <div className="flex flex-wrap items-start justify-between gap-3">
        ...
        <Link href="/admin/dashboard/inventory" className="mt-1 flex items-center gap-2 whitespace-nowrap font-nunito text-sm text-[#622581] transition-colors hover:text-[#4d1d67]">
          <ArrowLeft className="h-4 w-4" /> Back to Inventory Management
        </Link>
      </div>

      <div className="bg-white px-4 py-6 sm:px-9 sm:py-7">
        <h2 className="font-poppins text-base font-medium text-gray-800">Create Inventory Login</h2>
        <form onSubmit={handleSubmit} className="mt-6">
          <div className="grid grid-cols-1 gap-x-10 gap-y-4 md:grid-cols-2 lg:gap-x-16">
            ...
            <label className="font-nunito text-sm font-normal text-gray-800">Password<input required type="password" autoComplete="new-password" placeholder="Enter password" value={formData.password} onChange={(e) => updateField("password", e.target.value)} className="mt-1.5 w-full rounded-lg border border-gray-300 px-3 py-2.5 font-nunito text-sm text-gray-700 outline-none transition placeholder:text-gray-400 focus:border-[#622581] focus:ring-2 focus:ring-[#622581]/20" /></label>
          ...
          <div className="mt-8 flex justify-end">
            <button type="submit" className="min-w-[140px] rounded-lg bg-[#622581] px-8 py-2.5 font-poppins text-lg font-medium text-white transition-colors hover:bg-[#52206d]">Submit</button>
          </div>
        </form>
      </div>
    </div>
