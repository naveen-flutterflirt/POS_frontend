        ...
        <nav className={`flex-1 space-y-1 py-4 ${isSidebarOpen ? "px-3" : "px-2"}`}>
          {menuItems.map((item) => {
            const isActive = item.path
              ...
              : false;
            const isExpanded = item.name === "Category Management" ? isCategoryOpen : isCustomerOpen;
            const toggleGroup = item.name === "Category Management"
              ? () => setIsCategoryOpen((o) => !o)
              : () => setIsCustomerOpen((o) => !o);
                ...
                <div key={item.name}>
                  ...
                  >
                    ...
                    <div className="flex items-center gap-3">
                      ...
                      {isSidebarOpen && <span className="font-nunito text-sm font-medium">{item.name}</span>}
                    </div>
                    {isSidebarOpen && (
                    ...
                    <div className="ml-9 mt-1 space-y-1">
                      {item.subItems.map((sub) => {
                        ...
                        return (
                          ...
                          >
                            <span className={`absolute left-0 top-0 bottom-0 w-1 rounded-r-full bg-[#622581] transition-opacity duration-200 ${isSubActive ? "opacity-100" : "opacity-0 group-hover:opacity-100"}`} />
                            {sub.name}
                          </Link>
                        );
                      })}
        ...
        >
          ...
          <div className="flex items-center gap-1 sm:gap-4">
            <div className="relative hidden md:block">
              ...
              <input type="text" placeholder="Search" className="w-48 rounded-lg border border-gray-200 py-2 pl-9 pr-4 font-nunito text-sm outline-none transition focus:border-[#622581] focus:ring-2 focus:ring-[#622581]/30 lg:w-64" />
            </div>
