...
export default function AdminPaymentDetails() {
	...
	const handleEditSubmit = async (event: React.FormEvent) => {
        ...
        try {
            await put(`/invoices/${editFormData.id}`, editFormData);
	...
	const handleDeleteConfirm = async () => {
        ...
        try {
            await del(`/invoices/${selectedPayment.id}`);
            setPayments((currentPayments) =>
	...
	const updateField = (field: keyof Payment, value: string) => {
		setEditFormData((currentData) => currentData ? { ...currentData, [field]: value } : currentData);
	};
						...
						>
							{([
								...
								<label key={field} className="text-sm font-normal text-gray-600">
									{label}
									<input
										value={editFormData ? (editFormData[field] as string) : ''}
